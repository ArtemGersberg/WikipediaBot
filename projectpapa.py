...
async def age_process(message: types.Message, state: FSMContext):
    ...
    if aba=="Инструкция по пользованию":
        await message.answer("Этот бот предназначен для расчёта материала необходимого при производстве промышленных бетонных и полимерных полов.\nЕсли у вас возникли вопросы с использованием бота, то можете прочитать это и попробовать найти ответ на свой вопрос.\nПри нажатии кнопки /start, вам предложат выбрать тип полов, для которых вы сможете рассчитать количество материала.\nНажав на кнопку 'Полимерные полы', вам предложат выбрать тип покрытия полимерных полов, а затем фактуру полов. После выбора фактуры полов вы сможете нажать на  нужную кнопку для расчёта того или иного материала в зависимости от вашего выбора.\nЕсли у вас остались вопросы, можете написать администратору бота @igorg80.\nКоманды:\n/start - запустить бота.\n/help - инструкция по пользованию/связь с администрацией.\n/calculation - сразу перейти к расчётам.\n/requirements - нормативные требования к полам\nПриятного использования)",reply_markup=ReplyKeyboardRemove())

    elif aba=="Связь с администратором бота":
        await message.answer("Если у вас есть предложения или остались вопросы по функционалу бота, то можете написать администратору @igorg80\nСпасибо, что пользуетесь ботом)",reply_markup=ReplyKeyboardRemove())
    else:
        if aba=="/calculations":
                await message.answer("Выберите тип полов", reply_markup=lang_markup6)
                await state.set_state("Выбор вида полов")
...
async def age_process(message: types.Message, state: FSMContext):
    ...
    if ac2=="Требования к нанесению":
        ...
        await message.answer("Прочность основания на сжатие:для уличных условий применения для внутренних помещений при наличии движения транспорта для внутренних помещений при пешеходном движение не менее 30 МПаНе менее 25 МПаНе менее 20 МПа. Метод контроля-ГОСТ 22690, не менее шести замеров на каждые 100 м2(методами ударного импульса и отрыва со скалыванием).")
        await message.answer("Прочность основания на растяжение при отрыве: для уличных условий применения-для внутренних помещений при наличии движения транспорта- для внутренних помещений при пешеходном движение не менее 2,0МпаНе менее 1,5МПаНе менее 1,0Мпа (когезионный характер отрыва).Метод контроля-ГОСТ 22690, не менее шести замеров на каждые 100 м2.")
        await message.answer("Влажность основание не более 4% по массе, если иное не указано в технической документации производителя материалов покрытия.Метод контроля-ГОСТ 21718, не менее шести замеров на каждые 100 м2.")
        await message.answer("Отклонение от плоскости не более 2мм на двухметровой рейке.Метод контроля-Инструментальный, не менее шести замеров на каждые 100м2.")
...
async def set_language(message: types.Message, state: FSMContext):
    ...
    if lang_markup1 == "Расчет количества температурно-усадочных швов":
        ...
        await message.answer("На сколько рассчитать количество арматуры",reply_markup=lang_markup5)
...
async def set_language(message: types.Message, state: FSMContext):
    ...
    if h=="Расчёт количества арматуры(на квадратный метр)":
        await message.answer("Выберите скольки слойное армирование хотите сделать",reply_markup=lang_markup2)
...
async def set_language(message: types.Message, state: FSMContext):
    ...
    if lang_markup2 == "Однослойное армирование":
        ...
        await message.answer(
            "Эта функция считает количество арматуры на квадратный метр с двухслойным армированием\nДля того чтобы получить это количество, введите все данные, которые запрашивает бот и он выведет ответ",
            reply_markup=ReplyKeyboardRemove())
...
async def set_language(message: types.Message, state: FSMContext):
    ...
    if isinstance(f, str) and f.isdigit():
        ...
        await message.answer("Выберите скольки слойное армирование на всю площадь хотите сделать", reply_markup=lang_markup2)
